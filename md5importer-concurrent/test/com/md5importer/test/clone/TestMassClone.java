...
public class TestMassClone extends SimpleGame {
	...
	private void createClone(int index) {
		...
		((Spatial)node).setLocalTranslation(30*index, 0, 0);
	...
	private IMD5Node loadMesh() throws IOException {
		try {
			final URL url = this.getClass().getClassLoader().getResource("com/md5importer/test/data/marine.md5mesh");
			return this.importer.loadMesh(url, "Mesh");
		} finally {
	...
	private IMD5Anim loadAnim() throws IOException {
		try {
			final URL url = this.getClass().getClassLoader().getResource("com/md5importer/test/data/marine.md5anim");
